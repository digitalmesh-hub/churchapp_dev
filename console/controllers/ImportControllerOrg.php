 ...
 */
class ImportControllerOrg extends Controller
{
    ...
    private function collectFamilyData($familyId, $headOfFamily, $spouse, $dependants, $memberId, $allMembers)
    {
        ...
        $familyData['skipped_dependants'] = [];
        
        foreach ($dependants as $dependantData) {
            // Check if should be skipped
            $skipReason = null;
            if ($dependantData['Active'] !== 'Y') {
                $skipReason = 'Inactive (Active != Y)';
            } elseif ($dependantData['DeathStatus'] === 'Yes') {
                $skipReason = 'Deceased';
            }
            ...
            if ($skipReason) {
                ...
                $dependantInfo['skip_reason'] = $skipReason;
                $familyData['skipped_dependants'][] = $dependantInfo;
                continue;
            }
        ...
        // Remove skipped_dependants key if empty
        if (empty($familyData['skipped_dependants'])) {
            unset($familyData['skipped_dependants']);
        }
